...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async function () {
        ...
        const parser = await LanguageParser.get(editor.document.languageId);
        assert(parser);
        ...
        const decorations: vscode.DecorationOptions[] = treeView.map((line) => {
          return {
            range: new vscode.Range(
              line.startPosition.row + selection.start.line,
              0,
              line.startPosition.row + selection.start.line,
              0,
            ),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("vscode-textobjects.bugFile", async () => {
      ...
      fs.writeFileSync(path.join(bugsDir, new Date().toUTCString()), text, {
        encoding: "utf-8",
      });
    }),
  );
